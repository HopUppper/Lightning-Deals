...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Enforcement 1: Stateless DB-backed IP rate limiting gate (3 attempts per 30 minutes)
    const rateLimit = await checkRateLimit('password-reset', clientIp, 3, 30 * 60 * 1000);
    if (!rateLimit.allowed) {
      return {
        statusCode: 429,
        headers,
        body: JSON.stringify({ 
          error: `Too many password reset requests or verification attempts from this connection. Please try again in ${rateLimit.retryAfterMins} minutes.` 
        })
      };
    }
